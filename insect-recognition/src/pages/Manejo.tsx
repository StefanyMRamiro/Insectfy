...
function Manejo() {
  ...
  const [quantidadeInseticida, setQuantidadeInseticida] = useState(0); // Quantidade total de Inseticida em litros
  ...
  const calcular = () => {
    ...
    // Cálculo da quantidade de Inseticida
    const quantidadeTotalInseticida = taxaAplicacao * areaTotal; // Litros de Inseticida necessários

    // Atualizar estados
    setDensidadePopulacao(totalPopulacao);
    setQuantidadeInseticida(quantidadeTotalInseticida);
  };
  ...
  return (
    <div className="p-4">
      ...
      <label htmlFor="populacaoPor05m2" className="block mb-2 text-sm font-medium text-gray-900">População de insetos por 0,5 m²:</label>
      <input
        type="number"
        id="populacaoPor05m2"
      ...
      <div className="flex justify-center mt-4">
        <button 
          onClick={calcular}
          className="bg-amber-900 text-white font-semibold py-2 px-4 rounded"
        >
          Calcular
        </button>
      </div>
      ...
      {densidadePopulacao > 0 && (
        <div className="mt-4">
          ...
          <p>Densidade total de insetos: {densidadePopulacao} insetos</p>
          <p>Quantidade total de inseticidas: {quantidadeInseticida} litros</p>
        </div>
      )}
    </div>
  );
